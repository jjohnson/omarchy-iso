...
omarchy_iso_docker_image() {
  case "$1" in
    x86_64) echo "archlinux/archlinux:latest" ;;
    aarch64) echo "menci/archlinuxarm:latest" ;;
  esac
}
...
omarchy_iso_live_kernel() {
  case "$1" in
    x86_64) echo "linux-t2" ;;
    aarch64) echo "linux-aarch64" ;;
  esac
}

omarchy_iso_online_pacman_config() {
  local architecture="$1"
  local channel="$2"

  if [[ $architecture == "aarch64" ]]; then
    echo "/configs/pacman-online-${channel}-aarch64.conf"
  else
    echo "/configs/pacman-online-${channel}.conf"
  fi
}
...
omarchy_iso_prepare_package_list() {
  ...
  while IFS= read -r line || [[ -n $line ]]; do
    ...
    case "$package" in
      amd-ucode|intel-ucode) continue ;;
      linux) package=linux-aarch64 ;;
      linux-headers) package=linux-aarch64-headers ;;
      qemu-user-static-binfmt) package=qemu-user-binfmt ;;
    esac
