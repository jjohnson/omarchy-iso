#!/bin/bash
# Build packages from /omarchy-source and /omarchy-pkgs into the ISO mirror.

set -eo pipefail

offline_mirror_dir="${1:-}"
if [[ -z $offline_mirror_dir ]]; then
  echo "Usage: build-omarchy-packages.sh <offline-mirror-dir> [build-cache-dir]" >&2
  exit 1
fi
build_cache_dir="${2:-$offline_mirror_dir/../../build-dependencies}"
...
rm -rf "$work_dir"
mkdir -p "$work_dir" "$offline_mirror_dir" "$build_cache_dir"
...
package_recipe_fingerprint() {
  ...
  (
    ...
    if grep -q 'OMARCHY_SRC' "$package_source/PKGBUILD"; then
      cd /omarchy-source
      if git -c safe.directory=/omarchy-source rev-parse --is-inside-work-tree \
        >/dev/null 2>&1; then
        git -c safe.directory=/omarchy-source \
          ls-files -co --exclude-standard -z
      else
        # A mounted linked worktree has a .git file whose gitdir points to a
        # host-only path. Hash its visible tree directly, excluding that
        # unusable pointer, so local-source cache invalidation still works.
        LC_ALL=C find . -path './.git' -prune -o -type f -print0
      fi |
        LC_ALL=C sort -z |
        xargs -0 sha256sum
    fi
...
reuse_build_dependency() {
  ...
  local fingerprint_file="$build_cache_dir/$package.recipe.sha256"
  local manifest_file="$build_cache_dir/$package.archives"
  local archive_name
  ...
  while IFS= read -r archive_name; do
    ...
    [[ -f $build_cache_dir/$archive_name ]] || return 1
    package_files+=("$build_cache_dir/$archive_name")
  done < "$manifest_file"
...
persist_build_dependency() {
  ...
  if [[ -f $manifest_file ]]; then
    while IFS= read -r archive_name; do
      [[ -n $archive_name ]] || continue
      rm -f "$build_cache_dir/$archive_name"
    done < "$manifest_file"
  fi
  ...
  for package_file in "$@"; do
    ...
    cp "$package_file" "$build_cache_dir/$archive_name"
    printf '%s\n' "$archive_name" >> "$manifest_tmp"
    cached_files+=("$build_cache_dir/$archive_name")
  done
  mv "$manifest_tmp" "$manifest_file"
  printf '%s\n' "$recipe_fingerprint" > "$build_cache_dir/$package.recipe.sha256"
...
reuse_runtime_package() {
  ...
  local fingerprint_file="$build_cache_dir/$package.runtime.recipe.sha256"
  local manifest_file="$build_cache_dir/$package.runtime.archives"
  ...
  while IFS= read -r archive_name; do
    ...
    # Final mirror pruning can remove unneeded siblings emitted by a split
    # PKGBUILD. Reuse the surviving target archive instead of rebuilding the
    # entire recipe merely because an unrelated sibling is no longer cached.
    [[ -f $package_file ]] || continue
    package_archive_satisfies "$target" "$package_file" && satisfied=1
...
persist_runtime_package_metadata() {
  ...
  mv "$manifest_tmp" "$manifest_file"
  printf '%s\n' "$recipe_fingerprint" > "$build_cache_dir/$package.runtime.recipe.sha256"
}

copy_runtime_archives() {
  local target="$1"
  shift
  local package_file package_name destination
  local satisfied=""
...
build_package() {
  ...
  if [[ $target == "-" ]] && reuse_build_dependency "$package" "$recipe_fingerprint"; then
    return
  fi
  if [[ $target != "-" ]] && reuse_runtime_package "$target" "$package" "$recipe_fingerprint"; then
    return
  fi
  ...
  if [[ $target == "-" ]]; then
    ...
    persist_runtime_package_metadata "$package" "$recipe_fingerprint" "${persisted_files[@]}"
...
EOF
  ...
  while read -r target package dependency_mode extra; do
    ...
    target=$(resolve_package_token "$target")
    package=$(resolve_package_token "$package")
  ...
  rm -f "$offline_mirror_dir"/omarchy-local.db* "$offline_mirror_dir"/omarchy-local.files*
