...
if [[ $OMARCHY_ARCH == "aarch64" ]]; then
  ...
  rm -f "$build_cache_dir/airootfs/etc/pacman.d/hooks/99-omarchy-iso-arm64-kernel.hook"
  rm -f "$build_cache_dir/airootfs/usr/local/bin/omarchy-iso-stage-arm64-kernel"
...
# live initramfs.
arch_packages=("$(omarchy_iso_live_kernel "$OMARCHY_ARCH")" git gum jq openssl plymouth python-terminaltexteffects tzupdate omarchy-keyring "$OMARCHY_SETTINGS_PACKAGE" lvm2 cryptsetup parted)
